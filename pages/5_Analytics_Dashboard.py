    ...
    with st.expander(f"**{r['id']}** — {r.get('problem_statement','')[:80]}…", expanded=False):
        ...
        with col_r:
            ...
            if new_status != r.get("status"):
                if st.button("💾 Save", key=f"m4_save_{r['id']}", use_container_width=True):
        ...
        with st.expander(f"**{r['id']}** · ISO Compliance {pct}% ({passed}/{total} clauses) — {r.get('action_owner','—')}", expanded=False):
            ...
            with col_b:
                ...
                if decision != "— Pending —":
                    ...
                    if st.button("💾 Record Decision", key=f"iso_save_{r['id']}", use_container_width=True):
...
def _render_smart_questions(result: dict):
    ...
    if st.button("💾 Save Responses", use_container_width=False):
